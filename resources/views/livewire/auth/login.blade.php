...
new #[Layout('components.layouts.auth')] class extends Component {
    ...
    public function login(): void
    {
        ...
        $this->redirect(route('verify-code'), navigate: true);
    }
...
<div class="flex flex-col gap-6">
    ...
    <form wire:submit="login" class="flex flex-col gap-6">
        <!-- Email Address -->
        <flux:input wire:model="email" :label="__('Correo electronico')" type="email" required autofocus
        ...
        <!-- Remember Me -->
        <flux:checkbox wire:model="remember" :label="__('Recordar')" />
    ...
    @if (Route::has('register'))
        <div class="space-x-1 rtl:space-x-reverse text-center text-sm text-zinc-600 dark:text-zinc-400">
            ...
            <flux:link :href="route('register')" wire:navigate class="text-black hover:text-green-700 font-bold">
                {{ __('Registrarse') }}
            </flux:link>
        </div>
    @endif
